...
class UploadTests: XCTestCase {
    ...
    // Upload a Thing and check we get back the orbitID
    func testThingUpload() throws {
        ...
        XCTAssertNil(things[0].uploadID, "The uploadID should be unset afte upload")
...
extension UploadTests: URLSessionTaskDelegate {
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        try! Thing.uploadDidComplete(for: task.taskIdentifier) // FIXME: catch throw
        
        // UploadTests specific
...
extension UploadTests: URLSessionDataDelegate {
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        try! Thing.uploadDidReceive(for: dataTask.taskIdentifier, data: data) // FIXME: catch throw
    }
}
